...
<script>
    ...
    $(document).ready(function() {
        ...
        "ajax": {
            "url": "<?= site_url('Approve_cuti/ajax_list') ?>",
            "type": "POST",
            "data": function ( data ) {
...
function approve(params,paramsDua){
    $.ajax({
        type: "POST",
        url: "<?= base_url("Approve_cuti/getDataModal"); ?>",
        data: {
            idp:params,
            idcuti:paramsDua
        },
        ...
        success: function (res) {
            if(res.status == 1){
                ...
                $("#jatah_cuti_app").val(res.data[0].jatah_cuti+" Hari");
                $("#lama_cuti").val(res.data[0].lama+" Hari");
...
function simpan_approve() {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= base_url("approve_cuti/approve") ?>",
                type: "POST",
                data: Postdata,
                processData: false,
                contentType: false,
