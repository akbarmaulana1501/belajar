...
function add() {
    ...
    $('.modal-title').text('Pengajuan izin');


    $('#image-preview').hide();
    $('.id_pegawai').hide();
    $('[name="jns_izin"]').trigger("change.select2");
}

function save() {
            $('#btnSave').text('saving...'); //change button text
            $('#btnSave').attr('disabled',true); //set button disable 
            var url = "<?= base_url('izin/insertizin'); ?>";
...
function approve(params,paramsDua){
    $.ajax({
        type: "POST",
        url: "<?= base_url("izin/getDataModal"); ?>",
        data: {
            idp:paramsDua
        },
        dataType: "json",
...
<script>
    function loadJenis(){
        $.ajax({
            type: "GET",
            url: "<?= base_url("izin/getJenisIzin") ?>",
            dataType: "json",
            success: function (res) {
                    var opt = '';
                    for(var i=0;i<res.length;i++){
                            opt += '<option nilai="'+btoa(res[i].lama_jenis_izin)+'" value="'+res[i].id_jenis_izin+'">'+res[i].nm_jenis_izin+'</option>';
                        }
                        $('#jns_izin').append(opt);
            }
        });
    ...
    $("#jns_izin").on("change",(e) => {
        e.preventDefault();
        var nilai = $('option:selected', $("#jns_izin")).attr('nilai');
        $("#lama").val(atob(nilai));
    });
</script>
