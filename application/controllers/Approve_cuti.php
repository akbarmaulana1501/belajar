...
class Approve_cuti extends CI_Controller 
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('M_cutiApp','cutiApp');
    }
    ...
    public function index()
    {
        ...
        $data['m']              = 'Cuti';
        $data['ml']             = 'Approve Cuti';
        $app_role = $this->db->get_where('pegawai_approval',array("approval" => $this->session->userdata("id_pegawai")))->num_rows();
        if($app_role == 0){
            $this->blocked();
        }else{
            $this->template->load('templates/master','admin/approval/approval_cuti', $data);
        }
    }

    public function ajax_list(){
        $list = $this->cutiApp->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach($list as $cuti){
            ...
            $approval   = $this->cutiApp->get_by_id_pegawai_cuti($id_pegawai_cuti);
            if($cuti->status == 0){
            ...
            $row[] = '<a href="'.base_url().'approve_cuti/cetak/'.encrypt_url($cuti->id_pegawai_cuti).'" target="_blank" class="btn-xs btn-primary waves-effect waves-light"><i class="fas fa-print"> </i> Cetak </a>&nbsp;';
...
$output = array(
    "draw"              => $_POST['draw'],
    "recordsTotal"      => $this->cutiApp->count_all(),
    "recordsFiltered"   => $this->cutiApp->count_filtered(),
    "data"              => $data,
);
...
private function __createPrimaryKey($params){
    ...
    $newID = null;
    if($params === "approve"){
        $newID = $this->cutiApp->getMaxID()->newID;
        if($newID === NULL){
            $newID = 0;
        }
    }
...
public function approve() {
    ...
    $jatah = $this->cutiApp->getDataJumlahCuti($where['id_pegawai'])->jatah_cuti;
    ...
    if($data['status'] == 1){
        if($jatah < $lama){
            ...
            $this->cutiApp->update($where,$data,"pegawai_cuti");
            $this->cutiApp->insert($secondTable,"cuti_approval");
            $this->cutiApp->update(array("id_pegawai" => $where['id_pegawai']),array("jatah_cuti" => $finalCuti),"pegawai");
            $out['status'] = 1;
        }
    ...
    $id_pengaju = $this->cutiApp->getIdPengaju($secondTable['id_pegawai_cuti']);
    $email_pengaju = $this->cutiApp->getEmailPengaju($id_pengaju);
    $persetujuan = $this->cutiApp->getPesetujuan($secondTable['id_pegawai_cuti']);
    $this->_sendEmail($email_pengaju,$persetujuan);

    // echo json_encode($id_pengaju);
}
...
public function getDataModal(){
    ...
    $list           = $this->cutiApp->getDataModal($id,$idcuti);
    $out            = array();
    $out['status']  = 1;
    $out['data']    = $list;
    echo json_encode($out);
...
public function deleteCuti(){
    ...
    $do = $this->cutiApp->delete(array("id_pegawai_cuti" => $id),"pegawai_cuti");
    $out['status'] = 1;
    echo json_encode($out);
}
...
public function insertCuti(){
    ...
    $data['status'] = 0;
    $this->cutiApp->insert($data,"pegawai_cuti");
...
private function _validate(){
    ...
    //     $id = decrypt_url($this->input->post('idp'));
    //     $list = $this->cutiApp->getDatacetak($id);
    //     $out = array();
    //     $out['status'] = 1;
    //     $out['data'] = $list;
    //     echo json_encode($out);
    ...
    //     // $this->data['Pegawai_cuti'] = $this->cutiApp->getDataCetak($id_pegawai);
...
private function _sendEmail($email_tujuan,$persetujuan)
{

    $setconfig = $this->cutiApp->setting_smtp('1');
        ...
        $nama_approval = $this->cutiApp->getNamaApproval($this->session->userdata('id_pegawai'));

        // if (empty($persetujuan)) {
        //     $hasil_persetujuan = "DITOLAK";
        // } else {
        //     $hasil_persetujuan = "DISETUJUI";
        // }
        ...
        $this->email->message('Persetujuan Cuti dari Bpk/Ibu : ' . $nama_approval .
            '<br>memutuskan bahwa permohonan cuti <strong>' . $hasil_persetujuan . '</strong>');
    ...
    public function cetak($params){
        ...
        $dt = $this->cutiApp->getDataCetak($id_pegawai);
