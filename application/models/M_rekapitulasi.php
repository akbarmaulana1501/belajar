...
class M_rekapitulasi extends CI_Model 
{
	...
	public function getJumlahPegawaiPerPendidikan(){
		//SELECT pegawai.pend_terakhir, COUNT(pegawai.id_pegawai) FROM pegawai GROUP BY pegawai.pend_terakhir
		$this->db->select('pegawai.pend_terakhir, COUNT(pegawai.id_pegawai) as jumlah_pegawai');
		$this->db->from('pegawai');
		$this->db->group_by('pegawai.pend_terakhir');

		$query = $this->db->get();
		return $query->result();
	}

	public function getJumlahPegawaiPerAgama(){
		//SELECT agama, COUNT(agama) from pegawai GROUP BY agama
		$this->db->select('pegawai.agama, COUNT(pegawai.agama) as agama_pegawai');
		$this->db->from('pegawai');
		$this->db->group_by('pegawai.agama');

		$query = $this->db->get();
		return $query->result();
	}

	public function getJumlahPegawaiPerTahun(){
		//SELECT DATE_FORMAT(tgl_diangkat_pwtt, '%Y') AS tanggal_diangkat, COUNT(*) FROM pegawai GROUP BY DATE_FORMAT(tgl_diangkat_pwtt, '%Y-%m')

		// SELECT DATE_FORMAT(tgl_diangkat_pwtt, '%Y') AS tanggal_diangkat, COUNT(*) 
		// FROM pegawai 
		// WHERE YEAR(tgl_diangkat_pwtt) >= 1990 AND YEAR(tgl_diangkat_pwtt) <= YEAR(CURRENT_DATE)
		// GROUP BY DATE_FORMAT(tgl_diangkat_pwtt, '%Y')

		$this->db->select("DATE_FORMAT(tgl_diangkat_pwtt, '%Y') AS tanggal_diangkat, COUNT(*) as jumlah_pegawai");
		$this->db->from('pegawai');
		$this->db->where('YEAR(tgl_diangkat_pwtt) >=', 1990);
		$this->db->where('YEAR(tgl_diangkat_pwtt) <=', date('Y'));
		$this->db->group_by("DATE_FORMAT(tgl_diangkat_pwtt, '%Y')");
